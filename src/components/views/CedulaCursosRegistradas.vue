    ...
    <v-row justify="center" align="center" style="height: 70px">
      <v-layout row justify-start>
        <v-flex align-self-center xs1>
          <v-btn outlined style="background-color: #f46722; color: #ffffff" @click="newForm" v-if="role < 1"> Crear nueva cédula </v-btn>
        </v-flex>
      </v-layout>
    </v-row>
    <v-row>
      <v-card>
        ...
        <v-data-table :headers="headers" :items="cedulas" item-key="id" class="elevation-1" :search="search" multi-sort>
          ...
          <template v-slot:top>
            ...
            <v-dialog v-model="dialogValidate" persistent max-width="1200px">
              <v-card>
                ...
                <v-card-text>
                  <AddCedulaPreAutCursosModal :id="cedula_id" :mode="mode" @close="close" v-if="dialogValidate" />
                </v-card-text>
              </v-card>
            </v-dialog>
          ...
          <template v-slot:[`item.actions`]="{ item }">
            <v-btn text @click="validateItem(item.id, 3)"> <v-icon small>mdi-eye-outline</v-icon><small>Ver</small> </v-btn>
            <v-btn v-if="item.esValido_DA < 1 && item.esValido_DT < 1 && role < 1" text @click="editItem(item)">
              <v-icon small>mdi-pencil</v-icon><small>Editar</small>
            </v-btn>
            <v-btn v-if="item.esValido_DA < 1 && item.esValido_DT < 1 && role < 1" text @click="deleteItem(item)">
              <v-icon small>mdi-window-close</v-icon><small>Eliminar</small>
            </v-btn>
            <v-btn text @click="validateItem(item.id, 2)" v-if="role == 1 && item.esValido_DT < 1">
              <v-icon small class="mr-2">mdi-check-bold</v-icon>
              <small>Validar</small>
            </v-btn>
            <v-btn text @click="validateItem(item.id, 2)" v-if="role == 2 && item.esValido_DA < 1">
              <v-icon small class="mr-2">mdi-check-bold</v-icon>
              <small>Validar</small>
            </v-btn>
