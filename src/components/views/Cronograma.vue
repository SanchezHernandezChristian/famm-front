  ...
  <v-container fluid class="max-height">
    <v-row justify="center" align="center">
      <v-col cols="12" class="d-flex flex-row">
        ...
        <v-btn small color="orange" dark class="ma-2" @click="addCronograma" v-show="role > 0"> CREAR NUEVO </v-btn>
      </v-col>
    </v-row>
    <v-row>
      <v-data-table :headers="headers" :items="items_cronogramas" :items-per-page="15" item-key="id" class="elevation-1">
            ...
            <v-dialog v-model="dialog" persistent max-width="1200px">
              <v-card>
                <v-card-title>
                  ...
                  <span class="text-h5" v-if="mode == 1">Editar cronograma</span>
                  <span class="text-h5" v-if="mode == 2">Validar cronograma</span>
                  <span class="text-h5" v-if="mode == 3">Ver cronograma</span>
                </v-card-title>

                <v-card-text>
                  <FormCronograma :id="cronograma_id" :mode="mode" @close="close" v-if="dialog" />
                </v-card-text>
              </v-card>
            </v-dialog>
          </v-toolbar>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-btn text @click="editItem(item.cronograma.idCronograma, 3)"> <v-icon small class="mr-2"> mdi-eye </v-icon> <small>Ver</small> </v-btn>
          <v-btn text @click="editItem(item.cronograma.idCronograma, 1)" v-show="role > 0 && item.cronograma.valido < 1">
            <v-icon small class="mr-2">mdi-pencil</v-icon>
            <small>Editar</small>
          </v-btn>
          <v-btn text @click="editItem(item.cronograma.idCronograma, 2)" v-show="role < 1 && item.cronograma.valido < 1">
            <v-icon small class="mr-2">mdi-check-bold</v-icon>
            <small>Validar</small>
          </v-btn>
          <v-btn text @click="deleteItem(item.cronograma.idCronograma)" v-show="role > 0 && item.cronograma.valido < 1">
      ...
      this.$swal({
        ...
        .then((response) => {
          if (response && response.serverCode == 200) {
            this.$swal('Eliminado!', 'El cronograma se ha eliminado.', 'success');
            this.fetchCronogramas();
          }
        })
        .catch((error) => {
